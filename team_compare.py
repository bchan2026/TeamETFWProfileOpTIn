...
def render_team_compare(df: pd.DataFrame) -> None:
    ...
    with st.container():
        ...
        if "compare_league" not in st.session_state or st.session_state.compare_league not in leagues:
            st.session_state.compare_league = leagues[0]

        st.session_state.compare_league = c1.selectbox(
            "League",
            leagues,
            index=leagues.index(st.session_state.compare_league),
        )

        league_df = df[df["League"] == st.session_state.compare_league].copy()
        league_df = league_df.drop_duplicates(subset=["Team"], keep="first")

        team_list = ["None"] + sorted(league_df["Team"].dropna().unique())

        # Persistent team slots
        if "compare_slots" not in st.session_state:
            st.session_state.compare_slots = ["None"] * 6

        cols = c2.columns(6)
        new_slots = []

        for i in range(6):
            current = st.session_state.compare_slots[i]
        ...
        st.session_state.compare_slots = new_slots

        per_game = st.checkbox("Per Game Averages", value=False)
    ...
    # SELECTED TEAMS
    # -----------------------------------------------------
    selected_teams = [t for t in st.session_state.compare_slots if t != "None"]
